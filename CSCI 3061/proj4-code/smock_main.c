...
int main(int argc, char *argv[]) {
    ...
    while (1) { // Keep reading until we break out of loop
        ...
        if (strcmp("exit", input) == 0) {
            ...
            int val;
            scanf("%d", &val); // read in target value
            if (mat == NULL) {
                printf("Error: There is no active matrix\n");
            } else {
            ...
            char host[128], port[128], matrix_name[128];
            scanf("%s %s %s", host, port, matrix_name);
            if (mat != NULL) {
                printf("Error: You must clear the current matrix first\n");
            } else {
                mat = matrix_download_udp(host, port, matrix_name);
                if (mat == NULL) {
            ...
            if (mat != NULL) {
                ...
                mat = matrix_download_tcp(host, port, matrix_name);
                if (mat == NULL) {
