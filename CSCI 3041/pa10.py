...
def insert_task(task_heap,task):
    task_heap.append(task) #Add a task to the end of the heap
    i = len(task_heap) - 1 #assigns i to the last position where task is added
    while i > 0 and task_heap[(i - 1) // 2].priority < task_heap[i].priority: #Check to see if the added tasks priority is greater than the parent of the task
        task_heap[i], task_heap[(i - 1) // 2] = task_heap[(i - 1) // 2], task_heap[i] #If so, swap priority with parents.
        i = (i - 1) // 2 #update index to parents index
...
def max_heapify(A, i):
    ...
    if left < len(A) and A[left].priority > A[largest].priority:
        largest = left #If left child exists and priority is greater than the current, make the largest the left child

    if right < len(A) and A[right].priority > A[largest].priority:
        largest = right #if right child exists and priority is greater than the current, make the largest the right child

    if largest != i: #check to see if a swap is needed because the largest is different than the original index
        A[i], A[largest] = A[largest], A[i] #swap indexes
        max_heapify(A, largest) #recursively call max heapify on the subtree at largest, ensuring correct swapping
...
def extract_max_priority_task(task_heap):
    if len(task_heap) == 0:
        return None #make sure heap isnt empty

    max_priority_task = task_heap[0] #Find max priority task
    task_heap[0] = task_heap[-1] #make last element the max priority task
    task_heap.pop() #remove last element
    max_heapify(task_heap, 0) #restore max heap property, keeping it a max heap
...
def increase_task_priority(task_heap,i,new_priority):
    if new_priority < task_heap[i].priority:
        return #Check if current priority is greater than the new priority
    task_heap[i].priority = new_priority #update the current priority to the new one, if the new priority is greater than the current
    while i > 0 and task_heap[(i - 1) // 2].priority < task_heap[i].priority: #continues as long as current index isnt at the front, and current priority isnt greater than the parent
        task_heap[i], task_heap[(i - 1) // 2] = task_heap[(i - 1) // 2], task_heap[i] #swap parent and current tasks
        i = (i - 1) // 2 #update current index to parent index to continue comparisons
